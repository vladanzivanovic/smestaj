...
- **No implementation details.** Do not specify file paths, class names, interfaces, service IDs, Doctrine entity/column mappings, Symfony routes, Twig templates, JS controllers, MariaDB tables, or any other implementation-level construct unless the user explicitly named them. Inference about the smestaj codebase (PHP 8.4+, Symfony 8.1+, Twig, Encore, MariaDB) is the planner's job.
