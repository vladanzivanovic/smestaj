...
permission:
  ...
  bash:
    ...
    "docker compose exec lamp php bin/console debug:*": allow
    "docker compose exec lamp php bin/console config:dump-reference*": allow
---
...
- Routes / JS routes: <yes/no — does `composer route-locale-generate` need to run?>
...
## Validation
- [ ] `docker compose exec lamp ./vendor/bin/phpunit` passes
- [ ] `docker compose exec lamp php -l <touched file>` passes for each touched PHP file
- [ ] `docker compose exec lamp npm run dev` builds without errors (if frontend was touched)
- [ ] `docker compose exec lamp php bin/console cache:clear` succeeds
- [ ] curl check (inside `lamp` container) against `http://localhost/<route>` returns expected status
- [ ] Manual smoke via `http://localhost:9500/<route>` if UI-facing
...
- **Respect AGENTS.md** conventions:
  - PHP 7.4 / Symfony 4.4: `final` classes where sensible, constructor DI, typed params + return types, imported class names (no inline FQCN), Yoda conditions, thin controllers, service layer, Doctrine repositories for data access.
  ...
  - SOLID throughout.
- **Docker only** for any command the executor will later run. Never suggest host-level commands. The PHP/Apache service is named `lamp`; the DB service is `mysql`.
...
- **JS routes / translations:** if any route or translation is added/changed/removed, the plan MUST include `composer route-locale-generate` (or the equivalent `php bin/console fos:js-routing:dump` + `bazinga:js-translation:dump`).
- **PHP 7.4 / Symfony 4.4 only.** No PHP 8-only syntax. No Symfony 5+ APIs.
