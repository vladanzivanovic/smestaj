...
Every command below runs through `docker compose exec <service>`. Never on the host. No `php`, `composer`, `npm`, `npx`, `node`, `mariadb`, `mysql`, or `curl` on the host machine. If a dependency is missing inside a container, install it **inside the container**; remove it after use unless the project needs it persistently.
...
- `lamp` — PHP 7.4 + Apache + Node/npm (project root mounted at `/var/www/html`).
- `mysql` — MariaDB 10, root password `vlada123!!!`, database `smestaj`.
- `phpmyadmin` — DB UI on `http://localhost:9502`.
...
```bash
docker compose exec lamp php -l src/SiteBundle/Path/To/File.php
```

### 1.2 PHP code style (if php-cs-fixer is available)

```bash
docker compose exec lamp php vendor/bin/php-cs-fixer fix --dry-run --diff
```
...
```bash
docker compose exec lamp ./vendor/bin/phpunit --filter <TestClassName>
docker compose exec lamp ./vendor/bin/phpunit tests/SiteBundle/Path/To/SpecificTest.php
```
...
```bash
docker compose exec lamp php bin/console debug:router
docker compose exec lamp php bin/console debug:container <service-id>
docker compose exec lamp php bin/console debug:autowiring
```
...
```bash
docker compose exec lamp npm run build
```
...
If any route, locale, or JS-visible translation was added/changed/removed during Phase 2, verify the dump ran:

```bash
docker compose exec lamp composer route-locale-generate
```

This regenerates `web/js/fos_js_routes.json` and the Bazinga translation files. If the executor forgot it, run it and note the omission in the report — but `qa` is still allowed to call it (it is a build step, not a source edit).

---

## 6. End-to-end validation (curl, inside the container)

**Never run `curl` on the host.** Use PHP's curl functions via `docker compose exec lamp php -r "..."`.

### 6.1 Port reference

| Where | URL |
|---|---|
| Site from **inside** the `lamp` container (preferred for validation) | `http://localhost` |
| Site from host (manual checks only) | `http://localhost:9500` |
...
```bash
docker compose exec lamp php -r "
...
```bash
docker compose exec lamp php bin/console doctrine:migrations:migrate --no-interaction
```
...
```bash
docker compose exec mysql mariadb -u root -p'vlada123!!!' smestaj -e "SELECT COUNT(*) FROM ad;"
```
...
- **No `curl` binary on host** — use PHP's curl functions inside the `lamp` container.
