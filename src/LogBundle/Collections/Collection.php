...
namespace LogBundle\Collections;

use LogBundle\Exception\CollectionException;
...
class Collection implements \Iterator, \Countable, \ArrayAccess
{
    ...
    public static function createFromString($string, $className = null): Collection
    {
        $items = preg_split( '/\r\n|\r|\n/', $string);
        $array = [];
        ...
        $messageArray = [];
        foreach ($items as $item) {
            preg_match('/\[(\d{4})-(\d{2})-(\d{2}) (\d{2}):(\d{2}):(\d{2})\]/', $item, $dates);

            if ([] !== $dates) {
                if(0 < count($messageArray)) {
                    $array[] = implode(PHP_EOL, $messageArray);
                }
                $messageArray = [];
            }
