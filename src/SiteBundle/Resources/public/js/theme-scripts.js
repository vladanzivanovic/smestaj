...
(function($) {
    ...
    SoapPopup.prototype = {
        ...
        open: function(options, obj) {
            ...
            if ($(wrapObj).length < 1) {
                ...
                $("<div class='opacity-overlay' " + idStr + "><div class='container'><div class='popup-wrapper'><i class='fa fa-spinner fa-spin spinner'></i><div class='col-xs-12 popup-content'></div></div></div></div>").appendTo("body");
            }
            stp.wrap = $(wrapObj);
    ...
    $.fn.soapPopup = function(options, obj) {
        stp = new SoapPopup();
        stp.init();
		...
		$(document).on("click touchend", ".opacity-overlay [data-dismiss='modal'], .close-btn", function(e) {
            stp.close();
        });
...
//         AppHelperService.defaultOption(this);
//         var obj = tjq(this);
...
//             }
//         }).on('change', function () {
//             if(tjq(this).hasAndGetData('set-validation') === 'yes') {
//                 tjq(this).valid();
//             }
//         });
//     });
...
function displayPhotoGallery($item) {
    ...
    var dataSlider = $item.data('slide-show');
...
tjq(window).on('load', function() {
    ...
    if (enableChaser == 1 && tjq('#content').length > 0 && tjq('#main-menu ul.menu').length > 0) {
        ...
        tjq('<h1 class="logo navbar-brand"><a title="Smestaj" href="'+BASE_PATH+'"><img alt="" src="'+LOGO+'"></a></h1>').insertBefore('.chaser .menu');
    ...
    // filters option
    tjq(".filters-container .filters-option").on('click touchend', 'li', function(e) {
        e.preventDefault();
        e.stopImmediatePropagation();

        var oncePerList = tjq(this).parents('ul').hasAndGetData('check-once');

        if(oncePerList) {
            tjq(this).parents('ul').find('li.active').each(function () {
    ...
    if(tjq().waypoint && !stGlobals.isMobile) {
        ...
        tjq('.animated').waypoint(function() {
            ...
            // string with comma is passed then random animation will be used
            type = type.split(',');
...
// login box
tjq(document).on("click touchend", ".travelo-signup-box .signup-email", function(e) {
    e.preventDefault();
...
String.prototype.generateUrl = function ($withOutAppDev){
    var path = location.pathname,
        regex = /((web)\/)|(app.php|app_dev.php)/g,
        regexNonPage = /(api)|(uploads)/g,
        match = path.match(regex),
        relativeUrl = '';

    if(regexNonPage.test(this.toString())) {
    ...
    if(match) {
        if(match[0])
            relativeUrl += '/'+ match[0].slice(0, -1);
        if(match[1] && !$withOutAppDev)
            relativeUrl += '/'+ match[1];
    }
    relativeUrl += this;
